...
class TestCli:
    ...
    def test_full_crate_copies_payload_and_overwrites_metadata(self, tmp_path):
        ohrm_dir = tmp_path / "input" / "TEST-ohrm"
        sql_dir = ohrm_dir / "ohrm" / "web" / "sql"
        sql_dir.mkdir(parents=True)
        (sql_dir / "createOHRM.sql").write_text(
        ...
        # Add payload files and a stale metadata file
        objects_dir = ohrm_dir / "objects"
        objects_dir.mkdir()
        (objects_dir / "sample.txt").write_text("payload content")
        (ohrm_dir / "ro-crate-metadata.json").write_text('{"stale": true}')

        output_dir = tmp_path / "output"
        result = runner.invoke(app, [str(tmp_path / "input"), "-o", str(output_dir), "--full-crate"])
        assert result.exit_code == 0
        # Payload files are copied
        assert (output_dir / "TEST-ohrm" / "objects" / "sample.txt").read_text() == "payload content"
        # Metadata is freshly generated, not the stale copy
        metadata = json.loads((output_dir / "TEST-ohrm" / "ro-crate-metadata.json").read_text())
        assert "@context" in metadata
        assert "stale" not in metadata

    def test_default_mode_no_payload_copy(self, tmp_path):
        ohrm_dir = tmp_path / "input" / "TEST-ohrm"
        sql_dir = ohrm_dir / "ohrm" / "web" / "sql"
        sql_dir.mkdir(parents=True)
        (sql_dir / "createOHRM.sql").write_text(
        ...
        # Add payload files that should NOT be copied
        objects_dir = ohrm_dir / "objects"
        objects_dir.mkdir()
        (objects_dir / "sample.txt").write_text("payload content")
        ...
        assert result.exit_code == 0
        assert (output_dir / "TEST-ohrm" / "ro-crate-metadata.json").exists()
        assert not (output_dir / "TEST-ohrm" / "objects").exists()
